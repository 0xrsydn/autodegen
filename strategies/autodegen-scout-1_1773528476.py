...
class Strategy:
    name = "ema_21_51_hh_hl_vol75_close35_v1"
    description = (
        "EMA 21/51 crossover with HH+HL structure (lookback 8). "
        "Volume above 75% of 10-bar avg, close in upper 35%."
    )
    parameters = {
        "ema_fast": 21,
        "ema_slow": 51,
    ...
    def on_bar(self, bar, portfolio):
        ...
        # Volume filter
        vol_lookback = self.parameters["volume_lookback"]
        if len(self.volume_history) >= vol_lookback:
            avg_vol = sum(self.volume_history[-vol_lookback:]) / vol_lookback
            volume_confirmed = bar.volume > avg_vol * self.parameters["volume_threshold"]
        else:
        ...
        if bar_range > 0:
            ...
            close_position = 0.5
        bullish_close = close_position >= self.parameters["min_close_position"]
        
        # Entry: EMA cross up with full uptrend structure and filters
        if current_pos == 0:
            if trend_up and not self.prev_trend_up and uptrend_structure and volume_confirmed and bullish_close:
