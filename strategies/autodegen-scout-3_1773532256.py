...
class Strategy:
    name = "anti_overfit_shadow_trail18_v1"
    description = (
        "EMA 20/50 with shadow filter (0.4) + structure (lb10) + trail 0.018. "
        "Hypothesis: Slightly tighter trail may improve decay."
    )
    parameters = {
        "ema_fast": 20,
        "ema_slow": 50,
    ...
    def on_bar(self, bar, portfolio):
        ...
        if len(self.high_history) < max(lookback * 2, self.parameters["ema_slow"]):
            return []
        ...
        upper_shadow = bar.high - body_top
        range_val = bar.high - bar.low
        shadow_ratio = upper_shadow / range_val if range_val > 0 else 0
        good_shadow = shadow_ratio < self.parameters["shadow_threshold"]
        ...
        # Entry: EMA cross up with shadow filter AND structure
        if current_pos == 0:
            if trend_up and not self.prev_trend_up and good_shadow and uptrend_structure:
