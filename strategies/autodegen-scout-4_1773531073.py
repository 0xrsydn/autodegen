"""
autodegen strategy — Shadow + Body Ratio Filter
"""
...
class Strategy:
    name = "shadow_accel_v82"
    description = (
        "EMA 20/50 with HH+HL structure + shadow filter + body ratio filter (0.20). "
    )
    parameters = {
        "ema_fast": 20,
        "ema_slow": 50,
    ...
    def on_bar(self, bar, portfolio):
        ...
        no_rejection = upper_shadow_ratio <= self.parameters["max_upper_shadow"]
        
        # Body ratio filter (conviction)
        body = abs(bar.close - bar.open)
        body_ratio = body / bar_range if bar_range > 0 else 0
        high_conviction = body_ratio >= self.parameters["min_body_ratio"]
        
        # Entry: EMA cross up with structure + no rejection + high conviction
        if current_pos == 0:
            if (trend_up and not self.prev_trend_up and uptrend_structure 
                and no_rejection and high_conviction):
