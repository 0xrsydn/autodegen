...
class Strategy:
    name = "ema_20_50_hh_hl_lb10_shadow_v1"
    description = (
        "EMA 20/50 crossover with HH+HL structure (lookback 10) + shadow filter. "
        "Skip entry if upper shadow/range > 0.4."
    )
    parameters = {
        "ema_fast": 20,
        "ema_slow": 50,
    ...
    def on_bar(self, bar, portfolio):
        ...
        # Shadow ratio filter (microstructure)
        ...
        # Entry: EMA cross up with full uptrend structure + no rejection
        if current_pos == 0:
            if trend_up and not self.prev_trend_up and uptrend_structure and no_rejection:
