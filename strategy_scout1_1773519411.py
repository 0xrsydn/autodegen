...
class Strategy:
    name = "ema_22_50_roc_trail019"
    description = (
        "22/50 EMA cross with ROC filter and 1.9% trailing stop. "
        "Enter only when 5-bar ROC is positive (momentum confirmation)."
    )
    parameters = {
        "ema_fast": 22,
        "ema_slow": 50,
    ...
    def on_bar(self, bar, portfolio):
        ...
        if current_pos == 0:
            # Enter on crossover with positive ROC (momentum confirmation)
